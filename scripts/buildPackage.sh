...
function release() {
  ...
  for os_arch in "${OS_ARCH[@]}" ; do
    ...
    (
      env GOOS="${OS}" GOARCH="${ARCH}" CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X 'github.com/Nataliia5722/terraform-provider-azuredevops/version.ProviderVersion=v${VERSION}'" -o "${BUILD_ARTIFACT}"
